...
public class Shoot : IState
{
    ...
    public void Execute()
    {
        Debug.Log("type of atk "+atkbb.aiming+"allow atk?:"+atkbb.allowShoot);
        ...
        else if (atkbb.aiming == ArcherBlackBoard.Aim.Hold)
        {
            ...
            atkbb.force=Mathf.Lerp(3,atkbb.maxForce, completion);
            Debug.Log("at hold");
            return;
        }
        else
        { elapsedTime = 0; }
        if (atkbb.allowShoot&&atkbb.currentArrow!=null)
        {
            Debug.Log("shoot");
